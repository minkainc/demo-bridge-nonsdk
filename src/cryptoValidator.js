...
async function verifySignature(hash, proof) {
  ...
  if (proof.method !== 'ed25519-v2') {
    throw new Error(`Unsupported signature method, expected 'ed25519-v2', but got: '${proof.method}'`)
  }
  
  valid = await ed25519.verify(Buffer.from(proof.result, 'base64'),digest,Buffer.from(proof.public, 'base64'));
...
function createSignatureDigest(hash, custom) {
  const serializedCustomData = custom
    ? stringify(custom)
    : ''
  return crypto
